		...
		<div class="ui center aligned grid">
			<div class="column viewh">
				<div class="ui segment">
					<admin-head :active=1></admin-head>

					<picker-display @dateVal="getDateVal"></picker-display>

					<div class="ui action input fluid " style="margin-top: 15px;">
						<input type="text" style="height: 2.5rem;" placeholder="手机号 / 订单号" v-model="key">
						<div class="ui button" @click="search">搜索</div>
					</div>

					<scroll-view scroll-y="true" style="margin-top: 14px;height: 500px;">

						<div class="ui info message" v-if="data == null || data.length < 1">
							<div class="header">
								没有任何数据
							</div>
						</div>
						...
						<div id="listContents" class="ui ">
							<div class="ui segment " @click='check(item.id)' v-for="(item ,i) in data" :key="item.id">
								<div class="desc">
									<div>姓名：{{item.senderName}}</div>
									<div>手机号：{{item.senderPhoneNum}}</div>
									<div>下单时间：{{item.createDate}}</div>
									<div>订单状态：<span :class="{redColor:item.orderStatus == '已取消',okColor:item.orderStatus=='已完成'}">{{item.orderStatus}}</span></div>
								</div>
								<div class="stat">
									<div class="ui label">
...
<script>
	export default {
		...
		methods: {
			...
			check: function(id) {
				uni.navigateTo({
					url: '/pages/admin/take_detail?id=' + id
				});
			},
			search(){
			...
			getList(that) {
				that.$req.order.aorder('t', that.date, that.key,(res) => {
					let data = res.data;
					if (data.status) {
						that.data = data.data;
					} else {
